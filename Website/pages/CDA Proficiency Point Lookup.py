...
CSV_CANDIDATES = [
    ...
    APP_DIR.parent / CSV_NAME,          # repo-root/PPMid-March2025.csv
]
...
LEVEL_SYNONYMS = {"prechamp": ["prechamp", "pre-champ", "pre champ", "prechampionship"],
...
def load_points_csv(source) -> pd.DataFrame:
    ...
    # Detect header row present as first row
...
def find_header_for_combo(col_index: Dict[str, str], style: str, level: str, dance_full: str) -> Optional[str]:
    style_n = normalize(style); dance_n = normalize(dance_full)
    for lvl_n in possible_level_keys(level):
        t1 = f"{style_n} {lvl_n} {dance_n}"
        if t1 in col_index: return col_index[t1]
        t2 = f"{lvl_n} {style_n} {dance_n}"
